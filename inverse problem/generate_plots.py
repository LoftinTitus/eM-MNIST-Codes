...
def load_model_and_generate_plots(model_type='fno'):
    ...
    if os.path.exists(checkpoint_path):
        ...
        if 'train_losses' in checkpoint:
            ...
            print("⚠️  No training history in checkpoint")
    else:
    ...
    print(f"\n✅ All plots and visualizations generated successfully!")
...
def generate_training_history_plot(checkpoint):
    ...
    if not train_losses or not val_losses:
        print("⚠️  No loss data available in checkpoint")
        return
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            if predict_properties and isinstance(seg_output, tuple):
                seg_output = seg_output[0]  # Get segmentation output
...
def visualize_predictions(model, test_loader, device, num_materials, num_samples=5, save_path="../exports/predictions/", model_name="Model"):
    ...
    with torch.no_grad():
        ...
        if isinstance(outputs, tuple):
            outputs = outputs[0]  # Get segmentation output
...
if __name__ == "__main__":
    # You can specify which model to generate plots for
    import sys
    
    model_type = 'fno'  # Default to FNO
    if len(sys.argv) > 1:
